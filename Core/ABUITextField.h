...
@protocol ABUITextFieldDelegate <UITextFieldDelegate>
 ...
 */
- (void)textField:(ABUITextField *)textField didPreventTextChangeInRange:(NSRange)range replacementString:(NSString *)replacementString;
 ...
 */
@interface ABUITextField : UITextField

@property(nonatomic, weak) id<ABUITextFieldDelegate> dlg;
...
/**
 *  在使用 maximumTextLength 功能的时候，是否应该把文字长度按照 [NSString (ABUI) ABUI_lengthWhenCountingNonASCIICharacterAsTwo] 的方法来计算。
 *  默认为 NO。
 */
