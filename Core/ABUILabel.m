...
#import "ABUILabel.h"
NSString * const ABUILabelLinkTypeKey = @"linkType";
NSString * const ABUILabelRangeKey = @"range";
NSString * const ABUILabelLinkKey = @"link";
...
- (void)setupTextSystem
{
    ...
    _linkDetectionTypes = ABLinkTypeOptionAll;
...
- (void)setLinkDetectionTypes:(ABLinkTypeOption)linkDetectionTypes
{
...
- (NSDictionary *)linkAtPoint:(CGPoint)location
{
    ...
    {
        NSRange range = [[dictionary objectForKey:ABUILabelRangeKey] rangeValue];
...
- (NSDictionary*)attributesForLinkType:(ABLinkType)linkType
{
...
- (void)setAttributes:(NSDictionary*)attributes forLinkType:(ABLinkType)linkType
{
    if (attributes)
    {
...
- (void)updateTextStoreWithAttributedString:(NSAttributedString *)attributedString
{
    ...
    {
        attributedString = [ABUILabel sanitizeAttributedString:attributedString];
    }
...
- (NSArray *)getRangesForLinks:(NSAttributedString *)text
{
    ...
    if (self.linkDetectionTypes & ABLinkTypeOptionUserHandle)
    {
        [rangesForLinks addObjectsFromArray:[self getRangesForUserHandles:text.string]];
    }
    
    if (self.linkDetectionTypes & ABLinkTypeOptionHashtag)
    {
        [rangesForLinks addObjectsFromArray:[self getRangesForHashtags:text.string]];
    }
    
    if (self.linkDetectionTypes & ABLinkTypeOptionURL)
    {
...
- (NSArray *)getRangesForUserHandles:(NSString *)text
{
    ...
    for (NSTextCheckingResult *match in matches)
    {
        ...
        {
            [rangesForUserHandles addObject:@{ABUILabelLinkTypeKey : @(ABLinkTypeUserHandle),
                                              ABUILabelRangeKey : [NSValue valueWithRange:matchRange],
                                              ABUILabelLinkKey : matchString
                                            }];
        }
    }
...
- (NSArray *)getRangesForHashtags:(NSString *)text
{
    ...
    for (NSTextCheckingResult *match in matches)
    {
        ...
        if (![self ignoreMatch:matchString])
        {
            [rangesForHashtags addObject:@{ABUILabelLinkTypeKey : @(ABLinkTypeHashtag),
                                           ABUILabelRangeKey : [NSValue valueWithRange:matchRange],
                                           ABUILabelLinkKey : matchString,
                                        }];
        }
...
- (NSArray *)getRangesForURLs:(NSAttributedString *)text
{
    ...
    for (NSTextCheckingResult *match in matches)
    {
        ...
        if (![self ignoreMatch:realURL])
        {
            ...
            {
                [rangesForURLs addObject:@{ABUILabelLinkTypeKey : @(ABLinkTypeURL),
                                           ABUILabelRangeKey : [NSValue valueWithRange:matchRange],
                                           ABUILabelLinkKey : realURL,
                                        }];
            }
        }
    }
...
- (NSAttributedString *)addLinkAttributesToAttributedString:(NSAttributedString *)string linkRanges:(NSArray *)linkRanges
{
    ...
    {
        NSRange range = [[dictionary objectForKey:ABUILabelRangeKey] rangeValue];
        ABLinkType linkType = [dictionary[ABUILabelLinkTypeKey] unsignedIntegerValue];
        ...
        if (_systemURLStyle && ((ABLinkType)[dictionary[ABUILabelLinkTypeKey] unsignedIntegerValue] == ABLinkTypeURL))
        {
            // Add a link attribute using the stored link
            [attributedString addAttribute:NSLinkAttributeName value:dictionary[ABUILabelLinkKey] range:range];
        }
    }
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (touchedLink)
    {
        self.selectedRange = [[touchedLink objectForKey:ABUILabelRangeKey] rangeValue];
    }
...
- (void)handleTap:(UITapGestureRecognizer*)ges {
    ...
    if (touchedLink)
    {
        NSRange range = [[touchedLink objectForKey:ABUILabelRangeKey] rangeValue];
        NSString *touchedSubstring = [touchedLink objectForKey:ABUILabelLinkKey];
        ABLinkType linkType = (ABLinkType)[[touchedLink objectForKey:ABUILabelLinkTypeKey] intValue];
        
        [self receivedActionForLinkType:linkType string:touchedSubstring range:range];
    }
}

- (void)receivedActionForLinkType:(ABLinkType)linkType string:(NSString*)string range:(NSRange)range
{
    switch (linkType)
    {
    case ABLinkTypeUserHandle:
